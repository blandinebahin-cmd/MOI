...
## Étape B du runbook (2026-07-08) — résultat 32/0/32 : le modèle « reconstruction » est tranché (OBSERVÉ)

Après l'étape A (saisie `NbjCPN-1` vidée ✔, MAJCPN-1 remplacé par REGULCPN1 dans l'onglet PCCN01 ✔, code nettoyé ✔) et F5 : **CP N-1 = 32/0/32** (pris 0 ✔ via `CP.RegulPris = 3` toujours actif ; brut 6 326,67 strict ✔ ; colonne NbjCPN-1 disparue des EV ✔).

**Mécanisme définitivement tranché par la paire d'observations** (avec saisie −3 → 29/0/29 ; sans saisie −3 → 32/0/32) :
- Les compteurs se **reconstruisent à chaque calcul** = mois figés (validés) + EV/profils du mois courant.
- Le **Rep +3 fantôme est dans les données FIGÉES de juin** (mois payé) → aucune purge « one shot » depuis juillet ne peut l'effacer : l'écriture de juillet disparaît au recalcul suivant dès que sa saisie est vidée.
- **Correction d'une conclusion antérieure (erronée)** : la saisie `NbjCPN-1 = −3` n'était **pas** une « bombe » — `Affecte` **pose** −3 sur l'élément de juillet à chaque calcul (idempotent), il ne cumule pas. C'est au contraire le **contre-poids permanent requis** jusqu'à la clôture du mois.

**Décision** : re-brancher `MAJCPN-1` (onglet « + » du PCCN01) + re-saisir `NbjCPN-1 = −3` chez le salarié test, et **laisser les DEUX saisies (`CP.RegulPris = 3` et `NbjCPN-1 = −3`) et les DEUX profils en place jusqu'à la validation de juillet** — retrait des deux en août (les `Saisie` ne se reportent pas). Cible après F5 : 29/0/29. Résidu PA (−427,73 €) réapparaîtra avec le −3 de MAJCPN-1 : documenté, non bloquant. L'import des 86 (étape C) n'est pas concerné (aucun fantôme chez eux — une seule colonne `CP.RegulPris`).

## Vues « Solde des repos » mai + juin, tous salariés (2026-07-08) — 2ᵉ salarié pollué détecté + chiffrage des soldes évaporés

**OBSERVÉ (édition détaillée Solde des repos, mai 2026 puis juin 2026, dossier complet) :**

1. **BARRAL (matricule 1002634) = le salarié du test méthode 147, encore pollué** : mai = N-1 31/28,50/**2,50** ; juin = N-1 **31,50 / 0 / 31,50** au lieu de ~29/1/28 attendu. Signature exacte du test 147 du 06/07 (« avant 29/1/28 → après 31,50/0/31,50 : acquis +2,5 = report du solde ancien, pris 1→0 ») : **les écritures du brouillon 147 ont persisté dans les données compteur de juin**, comme le fantôme MAJCPN-1 de BERARD. Ma conclusion antérieure « labo confiné à BERARD » était **fausse**. À traiter en unitaire : détail de son solde de repos (datation des écritures), contre-poids `NbjCPN-1 = −2,50` sur juillet si le +2,5 tient au recalcul, et vérifier le sort de son CP du 15/05 (pris 1→0 par le test : est-il dans le fichier d'import ? sa ligne bulletin de juin payée est-elle intacte ?).
2. **⚠ Vérifier que la méthode 147 est bien revenue à 0** au niveau dossier (le test du 06/07 était sur brouillon, la remise à zéro n'a jamais été confirmée) — un 147=1 résiduel changerait le comportement de la clôture 2027.
3. **Critère de contrôle D affiné** : « solde = acquis » ne suffit pas — BARRAL passe ce test avec un acquis faux (31,50/31,50). Ajouter le contrôle de la **valeur d'acquis attendue** (29 = 25+4 ; 27 ; variantes ancienneté/fractionnement type 30) sur la colonne Acquis N-1 de l'EH.
4. **Soldes anciens N-1 évaporés à la clôture (sujet client, hors correction)** : total mai = acquis 3 211,50 / pris 2 388,00 / **solde 823,50 j** non consommés au 31/05 (provision N-1 associée 422 670,47 €). ~508 j étaient en réalité pris en mai (saisis juin → corrigés par l'import) ; le reliquat ≈ **315 j réellement non pris, disparus sans ligne de perte ni report** (ex. BOTTARD 22 j, CHAY 22,5, RIPOTEAU 28, PIRVA 20…). À porter à la note de synthèse : politique de perte des CP au 31/05 à faire valider par le client (risque prud'homal si les salariés n'ont pas été mis en mesure de les prendre).
5. Cohérences relevées : les « pris N » de mai (BAK 2, TISSANDIER 4, TREMBLAY 2, GIOVANNACCI 7, RUPPE 2, HOGUIN 2, MAYZEL 2, DELEUZE 0,5) = prises **anticipées légitimes** enregistrées sur mai → deviennent pris N-1 en juin, **hors fichier d'import** ✔ ; BERARD juin = 32/3/29 (fantôme connu) ✔ ; lignes vides (sorties/entrants) et SMATT PINELLI 14,50 (temps partiel probable) à exclure des attendus standard.

## État « avant import » (2026-07-08, checklist rapprochée des vues Solde des repos) — les 3 « oranges » sont résolus

**OBSERVÉ (checklist `correction_CP_juin2026_checklist.xlsx` + vue Solde des repos de mai) :** les 3 « divergents » s'expliquent entièrement par les **prises anticipées légitimes enregistrées sur MAI** (nouvelle période, avant clôture) qui s'additionnent aux prises de mai saisies sur juin :
- GIOVANNACCI : compteur 11 = **7 anticipés (mai)** + 4 (bulletin juin) → import 4 ✔, cible post-import **29/7/22** ;
- RUPPE : 3 = **2 (mai)** + 1 (juin) → import 1 ✔, cible **29/2/27** ;
- TISSANDIER : 12 = **4 (mai)** + 8 (juin) → import 8 ✔, cible **29/4/25**.
→ Plus d'« inspection individuelle » nécessaire : leurs valeurs d'import sont justes, leurs cibles ne sont **pas** 29/0/29 (les anticipés restent). Même logique validée sur BAK (2 anticipés, hors fichier, cible 29/2/27).
- 2 rouges (BOURDERY, DOLLOIS) : hors fichier ✔ (sorties/STC, compteurs soldés, + anticipés légitimes).
- BERARD dans la checklist avec 3 : déjà corrigé via profil — l'import re-enverra 3 → `Min(3, pris 0) = 0`, no-op ✔.
